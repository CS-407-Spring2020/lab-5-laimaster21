...
public class DBHelper {
    ...
    public void createTable(){
        sqLiteDatabase.execSQL("CREATE TABLE IF NOT EXISTS notes"+
                "(INTEGER PRIMARY KEY, username TEXT,date TEXT, title TEXT, content TEXT, src TEXT)");
    }

    public ArrayList<Note> readNotes(String username){
        createTable();
        Cursor c=sqLiteDatabase.rawQuery(String.format("SELECT * from names where username like '%s'",username),null);
        ...
        while(!c.isAfterLast()){
            ...
            Note note=new Note(date,username,content,title);
            noteList.add(note);
            c.moveToNext();
        }
        c.close();
